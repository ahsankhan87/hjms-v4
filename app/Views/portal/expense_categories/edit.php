...
<main class="space-y-4">
    ...
    <article class="rounded-xl border border-slate-200 bg-white px-4 py-3">
        <h3 class="text-sm font-semibold text-slate-800">Edit Expense Category</h3>
        <p class="text-xs text-slate-500">Update category details for the active season.</p>
    </article>

    <article class="rounded-xl border border-slate-200 bg-white p-4">
        <form method="post" action="<?= site_url('/expense-categories/update') ?>" class="space-y-3">
            <?= csrf_field() ?>
            ...
            <div class="flex flex-wrap items-center justify-end gap-2 border-t border-slate-100 pt-3">
                <a href="<?= site_url('/expense-categories') ?>" class="btn btn-md btn-secondary">Back</a>
