...
<main class="space-y-4">
    ...
    <article class="rounded-xl border border-slate-200 bg-white px-4 py-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div>
                <h3 class="text-sm font-semibold text-slate-800">Sales Categories</h3>
                <p class="text-xs text-slate-500">Manage category grouping for office sales.</p>
            </div>
            <a href="<?= site_url('/sales-categories/add') ?>" class="btn btn-md btn-primary"><i class="fa-solid fa-plus"></i><span>Add Category</span></a>
        </div>
    </article>
    ...
    <div class="list-card overflow-auto">
        <table class="list-table">
            ...
            <tbody>
                <?php if (empty($rows)): ?>
                    <tr>
                        <td colspan="7" class="px-3 py-6 text-center text-slate-500">No sales categories found.</td>
                    </tr>
                    <?php else: foreach ($rows as $row): ?>
                        <tr class="border-t border-slate-100">
                            ...
                            <td class="px-3 py-2"><?= esc((string) ((int) ($row['sale_count'] ?? 0))) ?></td>
                            ...
                            <td class="px-3 py-2">
                                <div class="flex items-center gap-2">
                                    <a href="<?= site_url('/sales-categories/' . (int) $row['id'] . '/edit') ?>" class="icon-btn" title="Edit Category"><i class="fa-solid fa-pen"></i></a>
                                    <form method="post" action="<?= site_url('/sales-categories/delete') ?>" class="inline">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="category_id" value="<?= esc((string) $row['id']) ?>">
                                        <button type="submit" class="icon-btn icon-btn-danger" onclick="return confirm('Delete this sales category?')" title="Delete Category"><i class="fa-solid fa-trash"></i></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
