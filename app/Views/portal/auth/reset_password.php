...
<main class="min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/60">
        ...
        <form method="post" action="<?= site_url('/reset-password') ?>" class="mt-6 space-y-4">
            <?= csrf_field() ?>
            <div>
            ...
            <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-300">Reset Password</button>
        </form>
