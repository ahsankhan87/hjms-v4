...
<main class="space-y-4">
    ...
    <form method="post" action="<?= site_url('/bookings/update') ?>">
        <?= csrf_field() ?>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-4">
            <div class="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center gap-2">
                ...
                <h3 class="text-sm font-semibold text-slate-800">Package Details &nbsp;<span class="text-xs font-normal text-slate-400">#<?= esc((string) ($row['id'] ?? '')) ?></span></h3>
            </div>
                ...
                <div>
                    ...
                    <select id="booking-package" name="package_id" required class="js-select2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        <?php foreach ($packages as $item): ?>
                            <option value="<?= esc($item['id']) ?>" <?= $packageValue === (string) $item['id'] ? 'selected' : '' ?>>
            ...
            <div class="p-5">
                ...
                <select id="booking-pilgrims" name="pilgrim_ids[]" multiple required class="js-select2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    <?php foreach ($pilgrims as $item): ?>
                        <option value="<?= esc($item['id']) ?>" <?= in_array((int) $item['id'], $selectedPilgrimIds, true) ? 'selected' : '' ?>>
                            #<?= esc($item['id']) ?> — <?= esc($item['first_name'] . ' ' . $item['last_name']) ?><?= !empty($item['passport_no']) ? ' (' . esc($item['passport_no']) . ')' : '' ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <p class="mt-2 text-xs text-slate-400"><i class="fa-solid fa-circle-info mr-1"></i>Only pilgrims not in another confirmed booking are shown.</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl shadow-md overflow-hidden mb-4">
            ...
            <div id="seats-warning" class="hidden px-5 pb-4">
                <div class="rounded-lg bg-rose-500/30 border border-rose-300/40 px-3 py-2 text-xs text-rose-100 flex items-center gap-2">
                    ...
                    <span id="seats-warning-text"></span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-4">
            ...
            <div class="p-5">
                <textarea name="remarks" rows="3" placeholder="Optional notes about this booking…" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"><?= esc(old('remarks', (string) ($row['remarks'] ?? ''))) ?></textarea>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <button type="submit" id="submit-btn" class="btn btn-md btn-primary px-8">
                <i class="fa-solid fa-floppy-disk mr-2"></i>Update Booking
            </button>
...
<script>
    (function($) {
        ...
        function refreshSummary() {
            ...
            // On edit: only block submit when count actually exceeds remaining.
            // "No seats configured" is a warning only — the booking already exists.
